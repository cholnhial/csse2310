...
void phase_six(char* input)
{
    ...
    append_to_secret_string(6, (char)(48+PHASE_SIX_CONST*depth%9));
    depth+=1;
    if(depth/PHASE_SIX_CONST/33 == 0) {
	phase_six(input);
	return;
    }
    secret_string_matches(input);
...
void phase_seven(char* input)
{
    ...
    */
    ...
    append_to_secret_string(7, fortuneList->next->next->next->len);
    if(isdigit(input[0])) {
	for(int i = 0; i < input[0]-43; ++i) {
	    fortuneList = fortuneList->next;
	}
	muteflip();
    }
    append_to_secret_string(7, fortuneList->next->next->next->next->len * 3);
    mute(0);
    fortuneList = fortuneList->next->next->next;
    append_to_secret_string(7, 
	    (int)((fortuneList->next->data)[7]));
    secret_string_matches(input + 1);
}
